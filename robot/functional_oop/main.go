...
import (
	...
	"clean-architecture/functional_oop/api"
	"clean-architecture/functional_oop/internal/domain"
	"clean-architecture/functional_oop/internal/purerobot"
)
...
func main() {
	...
	move := domain.ActionCommand(func(steps int) {
		...
		fmt.Printf("POS %v %v\n", state.ShowPosition().X, state.ShowPosition().Y)
	})
